...
import phd.collins.imls.agents.behaviours.CyclicAddStudyAreaBehaviour;
...
public class StudyAreaAgent extends Agent implements IMLSAgentsVocabulary {

	private static final long serialVersionUID = -1537556250101422856L;
	...
	protected void setup() {
		...
		//Get Ontology
		imlsOntology = StudyAreaOntology.getInstance();
		...
		serviceDesc.setType(STUDY_AREA_AGENT);
		serviceDesc.setOwnership(STUDY_AREA_AGENT_OWNER);
		...
		// Service name
		String wsigServiceName = STUDY_AREA_AGENT_BASENAME + ((int)(Math.random() * 100) + 1);
		...
		//Add Behaviour
		this.addBehaviour( new CyclicAddStudyAreaBehaviour() );
	}
